jQuery(document).ready(function() {
    ...
    $('#usuario-registro').submit(function(){
        ...
        if(n === '' || a === '' || co === '' || cl === 'da39a3ee5e6b4b0d3255bfef95601890afd80709'){
            if($('.registro-mensaje-error').css('display') === 'none'){
        ...
        $.ajax({
            type: "GET",
            url: '/',
            data: {PeticionAjaxPublico: 'f', f: 'usuario-registro', n:n, a:a, d:d, co:co, cl:cl},
            dataType: 'json'
            }).done(function(datos) {
    ...
    $('.boton-envio-domicilio').click(function(){
        ...
        if(cerrado && $('#form-carrito-datos-de-envio').css('display') === 'none'){
    ...
    $('.contenedor-producto').on('mouseenter', function() {
        var i = $(this).attr('data-idProducto');
    ...
    $('.boton-registro-newsletter').click(function(){
        ...
        if(c === ''){
            $('.newsletter-estado-home-error').html('Es obligatorio ingresar el correo');
	...
	$('.js_buscar_codigo').click(function(){
	 ...
	 $('.js_getrepairbyid').click(function(){
		var settings = {
		  ...
		  url: 'http://brillante.rolivencia.xyz/api/fix/reparacion.php',
	      data: {
			  action: 'getById', 
			  repairId: $('.js_getrepairbyid_value').first().val()
		  },
		  method: 'GET'
		}
		...
		$.ajax(settings).done(function (response) {
			...
			$('.status_result').find('.js_res_modelo').first().html(response.modelo);
			$('.status_result').find('.js_res_marca').first().html(response.marca);
			$('.status_result').find('.js_res_ingreso').first().html(response.fechaIngreso);
			//$('.status_result').find('.js_res_entrega').first().html(response.fechaIngreso);
			$('.status_result').find('.js_res_estado').first().html(response.estado);
			$('.status_result').find('.js_res_precio').first().html(response.precioReparacion);
			$('.status_result').find('.js_res_nota').first().html(response.nota);
			$('.status_result').show();
			$('.status_form').hide();
		}); 
